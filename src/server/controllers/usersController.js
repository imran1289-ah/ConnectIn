...
const removeConnection = async(req, res) => {
    ...
    try {
        const removedUser = await User.findOneAndUpdate({ _id: userID }, { $pull: { connections: { userID: connectionID } } });
        const removedUserInConnectionSide = await User.findOneAndUpdate({ _id: connectionID }, { $pull: { connections: { userID: userID } } });
        return res
            .status(200)
            .json(
